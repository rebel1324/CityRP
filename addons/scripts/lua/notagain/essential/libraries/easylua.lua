...
function easylua.FindEntity(str)
    ...
    if str:sub(1, 1) == "#" then
        ...
        if #str > 0 then
            ...
            if found then return CreateAllFunction(function(v) return v:IsPlayer() and v:Team() == found end) end
            ...
            if found then return CreateAllFunction(function(v) return v:GetClass():lower() == found end) end
        end
    end
...
function easylua.Start(ply)
    if started then
        Msg"[ELua] "
        print("Session not ended for ", _G.me or (easylua.vars and easylua.vars.me), ", restarting session for", ply)
        easylua.End()
    end
    ...
    if vars.last and istable(vars.last) then
        vars.last.last = nil
    end
...
function easylua.RunLua(ply, code, env_name)
    ...
    env_name = env_name or string.format("%s", tostring(IsValid(ply) and ply:IsPlayer() and "[" .. ply:SteamID():gsub("STEAM_", "") .. "]" .. ply:Name() or ply))
...
hook.Add("LuaDevProcess", "easylua", function(stage, script, info, extra, func)
    if stage == STAGE_PREPROCESS then
        ...
        if not istable(extra) or not IsValid(extra.ply) or not script or extra.easylua == false then return end
        insession = true

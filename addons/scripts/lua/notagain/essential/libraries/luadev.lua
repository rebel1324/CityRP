...
function luadev.Run(script, info, extra)
    ...
    if CLIENT and not extra and info and istable(info) then
        return luadev.RunOnSelf(script, "COMPAT", {
            ply = info.ply
        })
    end
    ...
    if not func then
        if luadev.compileerr then return false, "Syntax error: " .. tostring(luadev.compileerr) end
    end
...
function luadev.CanLuaDev(ply, script, command, target, target_ply, extra)
    ...
    if target == luadev.TO_CLIENT and (target_ply == ply or (target_ply and istable(target_ply) and target_ply[1] == ply and table.Count(target_ply) == 1)) then
...
if SERVER then
    ...
    function luadev.RunOnClients(script, who, extra)
        if not who and extra and isentity(extra) then
            extra = {
                ply = extra
            }
        end
    ...
    function luadev._ReceivedData(_, ply)
        ...
        if not istable(extra) then return luadev.RejectCommand(ply, "bad extra table") end
        extra.ply = ply
        if not luadev.CanLuaDev(ply, script, nil, target, target_ply, extra) then return luadev.RejectCommand(ply) end
        --	if luadev.TransmitHook(data)~=nil then return end
        local identifier = luadev.GetPlayerIdentifier(ply, info)
        local ok, err

        if target == luadev.TO_SERVER then
            ok, err = luadev.RunOnServer(script, identifier, extra)
        elseif target == luadev.TO_CLIENT then
            ok, err = luadev.RunOnClient(script, target_ply, identifier, extra)
        elseif target == luadev.TO_CLIENTS then
            ok, err = luadev.RunOnClients(script, identifier, extra)
        elseif target == luadev.TO_SHARED then
...
if CLIENT then
    ...
    function luadev.ToServer(data)
        ...
        if data.extra then
            ...
            if table.Count(data.extra) == 0 then
                data.extra = nil
            end
        end

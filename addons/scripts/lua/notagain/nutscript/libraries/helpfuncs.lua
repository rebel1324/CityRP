...
function util.DecimalToIP(iIP)
    ...
    return floor(iIP / 16777216) .. dot .. floor(t1 / 65536) .. dot .. floor(t2 / 256) .. dot .. floor(t2 % 256)
end
...
-- Name & Path
---------------------------------------
GetScriptName = GetScriptName or function() return debug.getinfo(debug.getinfo(2, "f").func).short_src end
...
-- thanks avon
function ents.FindInsideRotatedBox(centre, min, max, ang)
...
function FindError(str, wide)
    ...
    if not linenum then
        error("linenum conversion failed")
    end
...
function Player:BotMimic(stop)
    if stop == false then return RunConsoleCommand("bot_mimic", tostring(-1)) end

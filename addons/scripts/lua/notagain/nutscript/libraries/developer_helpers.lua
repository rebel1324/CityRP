...
ENTITY.DataTbl = function(ent)
    ...
    local function _dodiff(diff, s, f, rev, k, v)
        ...
        elseif f ~= v then
            if isvector(f) then
                if f.x == v.x and f.y == v.y and f.z == v.z then return end
            elseif isangle(f) then
                if f.p == v.p and f.y == v.y and f.r == v.r then return end
            end
...
function asraw(t)
    local mt = {}
    mt.__index = function(self, k) return rawget(t, k) end
...
-- ConVars
CONVAR.Set = function(a, b, ...)
    RunConsoleCommand(a:GetName(), b, ...)
end

CONVAR.__tostring = function(self) return 'GetConVar("' .. (self.GetName and self:GetName() or "unknown") .. '"):Set("' .. (self.GetString and self:GetString() or "") .. '")' end
cvar = GetConVar
CVar = GetConVar
Cvar = GetConVar
-- typomod
...
addmetatable(coroutine, {
    __call = function(tbl, a, ...) return coroutine.create(a, ...) end
})

...
do
    ...
    local function request(url, callback, method, timeout, post_data, user_agent, binary)
        ...
        function socket:OnClose()
            ...
            local ok, err = xpcall(callback, on_error, {
                content = content,
                header = header
            })
...
do
    ...
    function NULL:__index(key)
        ...
        if key == "Type" then return "null" end
    ...
    local function add_options(tbl)
        for func_name, key in pairs(options) do
            ...
            tbl["Get" .. func_name] = function(self, val) return self[func_name] end
        end
    end
    ...
    do
        ...
        function CLIENT:OnTimeout(count)
        end
        ...
        function CLIENT:OnError(msg)
            self:Remove()
        end

        function CLIENT:OnSend(data, bytes, b, c, d)
        end
    ...
    do
        ...
        local DUMMY = {}
        DUMMY.__index = DUMMY
        DUMMY.__tostring = function(s) return string.format("dummy_client_%s[%s][%s]", "udp", s.ip or "nil", s.port or "nil") end
        ...
        function SERVER:OnReceive(data, client)
        end

        function SERVER:OnClientError(client, err)
        end

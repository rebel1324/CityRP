...
local enc = function(dat)
    if isentity(dat) then
        return dat:SteamID64() or util.SteamID64(dat:SteamID())
    end
...
local function getkeys(additional)
    if not istable(additional) then
        if isstring(additional) then
            return additional
        end

        if isentity(additional) then
            return enc(additional)
        end
...
local function API(interface, method, version, additional)
    ...
    if not data then
        error("Invalid method version: " .. version)
    end
    ...
    local url = string.format('http://api.steampowered.com/%s/%s/v%.4d/?%s', interface, method, version or 1, add)
...
local function SteamAPI(interface, method, version, additional, cb, err)
    ...
    local function callback(body, bodylen, headers, code)
        if code ~= 200 or not body or bodylen == 0 then return err and err(code, body) end
        local ok, dec = pcall(decode, body)
        if not ok then return err and err(dec) end
        cb(dec)
    end
...
function GetPlayerBans(pl, cb, err)
    SteamAPI('ISteamUser', 'GetPlayerBans', 1, {
        steamids = pl
    }, cb, err)
end
...
function GetPlayerSummaries(pl, cb, err)
    SteamAPI('ISteamUser', 'GetPlayerSummaries', 2, {
        steamids = pl
    }, cb, err)
end

function UpToDateCheck(cb, err)
    SteamAPI('ISteamApps', 'UpToDateCheck', 1, {
        appid = 4000,
        version = file.Read('steam.inf', 'GAME'):match("ersion=([%.%d]+)"):gsub("%.", "")
    }, function(resp)
        resp = resp and resp.response
        if not resp or resp.success == false or resp.error then return err and err(resp and resp.error, resp and resp.success) end
        cb(resp.up_to_date, resp.version_is_listable)
    end, err)
end

function GetNumberOfCurrentPlayers(cb, err, appid)
    SteamAPI('ISteamUserStats', 'GetNumberOfCurrentPlayers', 1, {
        appid = appid or 4000
    }, function(resp)
        resp = resp and resp.response
        if not resp or resp.result == 0 or resp.error then return err and err(resp and resp.error, resp and resp.result) end
        cb(resp.player_count)
    end, err)
end

function GetServersAtAddress(addr, cb, err)
    SteamAPI('ISteamApps', 'GetServersAtAddress', 1, {
        addr = addr
    }, function(resp)
...
function GetPublishedFileDetails(fileid, cb, err)
    SteamAPI('ISteamRemoteStorage', 'GetPublishedFileDetails', 1, {
        itemcount = 1,
        ['publishedfileids[0]'] = tostring(fileid)
    }, function(resp)
        resp = resp and resp.response or resp
        if not resp or resp.result ~= 1 or resp.error then return err and err(resp and resp.error, resp and resp.result) end

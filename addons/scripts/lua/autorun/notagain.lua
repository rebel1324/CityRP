...
do
    local function load_path(path)
        if not file.Exists(path, "LUA") then return nil, "unable to find " .. path end
    ...
    function notagain.GetLibrary(name, ...)
        --print("REQUIRE: ", name)
        if notagain.loaded_libraries[name] then return notagain.loaded_libraries[name] end
        local func
        local errors = ""

        if not func then
            local addon_tries = {"libraries/%s.lua", "libraries/client/%s.lua", "libraries/server/%s.lua", "%s.lua"}
        ...
        if func == nil then return nil, errors end
        local ok, lib = pcall(func, ...)
        if ok == false then return nil, lib end
        if lib == nil then return nil, "library " .. name .. " returns nil" end

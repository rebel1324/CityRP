...
chatexp.Modes = {
    {
        Name = "Default",
        Filter = function(send, ply) return true end,
    ...
    {
        Name = "Team",
        Filter = function(send, ply) return send:Team() == ply:Team() end,
        Handle = function(tbl, ply, msg, dead, mode_data)
            if dead then
                tbl[#tbl + 1] = color_red
                tbl[#tbl + 1] = "*DEAD* "
    ...
    function chatexp.SayAs(ply, data, mode, mode_data)
        ...
        elseif msgmode.Filter then
            for k, v in next, player.GetHumans() do
                if msgmode.Filter(ply, v) ~= false then
                    filter[#filter + 1] = v
                end
            end
        else
        ...
        if not table.HasValue(filter, ply) then
            filter[#filter + 1] = ply
        end
        ...
        if not cdata then
            Msg"CEXP "
            print"Failed to re-compress message."

...
local function recursive_init(tbl, meta, ...)
    if meta.__baseclass then
        recursive_init(tbl, meta.__baseclass, ...)
    end
...
function class:register(name, meta, base, noGlobal, tbl)
    ...
    if not noGlobal then
        ...
        env[name] = function(...) return class:new(name, ...) end
    end
end
...
function class:makeFunction(n)
    _G[n] = function(...) return class:new(n, ...) end
end

...
local keked = function()
	...
	local meta = FindMetaTable("Player")
	 ...
	 if (CLIENT) then
		...
		local function TFA_CSGO(wep, vm)
			...
			local borke = niggers[wep:GetClass()] 
			if (borke) then
		...
		local function flashlightThink(client)
			...
			if (NV_Status and NV_NIGGERTYPE > 1) then
				...
				if (wep:IsValid()) then
					...
					if (IsValid(vm) and
						...
						if (wep.CSGO) then
							...
							if (!funckingMelee[wep:GetClass()]) then
								local att = vm:GetAttachment(1)
		...
		hook.Add("PostPlayerDraw", "lolfag", function(client)
			flashlightThink(client)
		end)

		hook.Add("PreDrawTranslucentRenderables", "customFlashlight", function()
		...
		hook.Add("PlayerDeath", "asdfasdfasdfasdg", function(client)
 ...
 timer.Simple(1, keked)
 hook.Add("OnReloaded", "dogemems", function()
	timer.Simple(1, keked)
 end)

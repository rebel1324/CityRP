...
function FPP.AntiSpam.CreateEntity(ply, ent, IsDuplicate)
    if not tobool(FPP.Settings.FPP_ANTISPAM1.toggle) then
        return
    end
    ...
    if tobool(FPP.Settings.FPP_ANTISPAM1.bigpropantispam) and phys:GetVolume() and phys:GetVolume() > math.pow(10, FPP.Settings.FPP_ANTISPAM1.bigpropsize) and not string.find(class, "constraint") and not string.find(class, "hinge") and not string.find(class, "magnet") and not string.find(class, "collision") and not blacklist[class] then
    ...
    if not IsDuplicate and not blacklist[class] then
        ...
        if ply.FPPAntiSpamCount >= FPP.Settings.FPP_ANTISPAM1.smallpropghostlimit and ply.FPPAntiSpamCount <= FPP.Settings.FPP_ANTISPAM1.smallpropdenylimit and not ent:IsVehicle() then
            --[[Vehicles don't like being ghosted, they tend to crash the server]]
            FPP.AntiSpam.GhostFreeze(ent, phys)
...
function FPP.AntiSpam.DuplicatorSpam(ply)
    ...
    timer.Simple(ply.FPPAntiSpamLastDuplicate / FPP.Settings.FPP_ANTISPAM1.duplicatorlimit, function()
        if IsValid(ply) then
            ply.FPPAntiSpamLastDuplicate = ply.FPPAntiSpamLastDuplicate - 1
        end
    end)
...
local function e2AntiMinge()
    ...
    if not e2func or not e2func[3] then
        return
    end
    ...
    e2func[3] = function(self, args, ...)
        if not tobool(FPP.Settings.FPP_GLOBALSETTINGS1.antie2minge) then
            return applyForce(self, args, ...)
        end

        local ent = args[2][1](self, args[2]) -- Assumption: args[2][1] is a function

        if not IsValid(ent) or ent:CPPIGetOwner() ~= self.player then
            return
        end
...
hook.Add("InitPostEntity", "FPP.InitializeAntiMinge", function()
    ...
    function DoPlayerEntitySpawn(ply, ...)
        ...
        if not tobool(FPP.Settings.FPP_ANTISPAM1.antispawninprop) then
            return ent
        end

        local PropInProp = IsEmpty(ent)

        if not IsValid(PropInProp) then
            return ent
        end

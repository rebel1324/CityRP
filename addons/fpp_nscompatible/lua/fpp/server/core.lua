...
local function isBlocked(model)
    if model == "" or not FPP.Settings or not FPP.Settings.FPP_BLOCKMODELSETTINGS1 or not tobool(FPP.Settings.FPP_BLOCKMODELSETTINGS1.toggle) or not FPP.BlockedModels or not model then
        return
    end
...
if cleanup then
    ...
    function cleanup.Add(ply, Type, ent)
        ...
        if (not fuckoff[ent:GetClass()]) then
    ...
    function undo.Finish(...)
        if IsValid(UndoPlayer) then
            for k, v in pairs(Undo) do
                if (not fuckoff[v:GetClass()]) then
...
function FPP.Protect.PhysgunPickup(ply, ent)
    if not tobool(FPP.Settings.FPP_PHYSGUN1.toggle) then
        if FPP.UnGhost then
            FPP.UnGhost(ply, ent)
        end
...
function FPP.Protect.PhysgunReload(weapon, ply)
    if not tobool(FPP.Settings.FPP_PHYSGUN1.reloadprotection) then
        return
    end
    ...
    --If I return true, I will break the double reload
...
function FPP.Protect.GravGunPickup(ply, ent)
    if not tobool(FPP.Settings.FPP_GRAVGUN1.toggle) then
        return
    end
    ...
    elseif ent.GravGunPickup ~= nil then
        if ent.GravGunPickup == false then
            DropEntityIfHeld(ent)
        end
    ...
    if FPP.UnGhost and cantouch then
        FPP.UnGhost(ply, ent)
    end
...
function FPP.Protect.CanGravGunPickup(ply, ent)
    if not tobool(FPP.Settings.FPP_GRAVGUN1.toggle) or not IsValid(ent) then
        return
    end
...
function FPP.Protect.PlayerUse(ply, ent)
    if not tobool(FPP.Settings.FPP_PLAYERUSE1.toggle) then
        return
    end
...
function FPP.Protect.EntityDamage(ent, dmginfo)
    ...
    if not tobool(FPP.Settings.FPP_ENTITYDAMAGE1.toggle) then
        return
    end
    ...
    if not attacker:IsPlayer() then
        if not tobool(FPP.Settings.FPP_ENTITYDAMAGE1.protectpropdamage) then
            return
        end
        ...
        if IsValid(attackerOwner) and IsValid(entOwner) then
            ...
            if not cantouch then
                ...
                timer.Simple(1, function()
                    if not ent.FPPAntiDamageWorld then
                        return
                    end
...
timer.Simple(5, function()
    for k, v in pairs(weapons.GetList()) do
        if v.ClassName then
            allweapons[string.lower(v.ClassName or "")] = true
        end
    end
end)
...
local invalidToolData = {
    ["model"] = {"*", "\\"},
    ["material"] = {"*", "\\", " ", "effects/highfive_red", "pp/copy", ".v", "skybox/"},
    ["sound"] = {"?", " "},
    ["soundname"] = {" ", "?"},
    ["tracer"] = {"dof_node"},
    ["door_class"] = {"env_laser"},
    -- Limit wheel torque
    ["rx"] = 360,
    ["ry"] = 360,
    ["rz"] = 360
...
function FPP.Protect.CanTool(ply, trace, tool, ENT)
    ...
    if FPP.RestrictedToolsPlayers[tool] and FPP.RestrictedToolsPlayers[tool][SteamID] ~= nil then
        if FPP.RestrictedToolsPlayers[tool][SteamID] == false then
            FPP.Notify(ply, "Toolgun restricted for you!", false)
            --If someone is allowed, then he's allowed even though he's not admin, so don't check for further restrictions
    ...
    if not ignoreGeneralRestrictTool then
        ...
        if FPP.RestrictedTools[tool] then
            if tonumber(FPP.RestrictedTools[tool].admin) == 1 and not ply:IsAdmin() then
                ...
                -- If the person is not in the BUT has admin access, he should be able to use the tool
    ...
    if tool ~= "adv_duplicator" and tool ~= "duplicator" and tool ~= "advdupe2" then
        return
    end

    if not ENT and not FPP.AntiSpam.DuplicatorSpam(ply) then
        return false
    end

    local EntTable = (tool == "adv_duplicator" and ply:GetActiveWeapon():GetToolObject().Entities) or (tool == "advdupe2" and ply.AdvDupe2 and ply.AdvDupe2.Entities) or (tool == "duplicator" and ply.CurrentDupe and ply.CurrentDupe.Entities)
...
local function freezeDisconnected(ply)
    ...
    for _, ent in pairs(ents.GetAll()) do
        ...
        if not IsValid(ent) or ent.FPPOwnerID ~= SteamID or ent:GetPersistent() or not physObj:IsValid() then
            continue
        end
...
function FPP.PlayerDisconnect(ply)
    ...
    if ply.FPPFallbackOwner then
        ...
        for k, v in pairs(ents.GetAll()) do
            if not IsValid(v) or v.FPPOwnerID ~= SteamID or v:GetPersistent() then
                continue
            end
    ...
    if not tobool(FPP.Settings.FPP_GLOBALSETTINGS1.cleanupdisconnected) or not FPP.Settings.FPP_GLOBALSETTINGS1.cleanupdisconnectedtime then
        return
    end

    if ply:IsAdmin() and not tobool(FPP.Settings.FPP_GLOBALSETTINGS1.cleanupadmin) then
        return
    end
...
function FPP.PlayerInitialSpawn(ply)
    ...
    if FPP.DisconnectedPlayers[SteamID] then
        for k, v in pairs(ents.GetAll()) do
...
function constraint.CanConstrain(ent, bone)
    if IsValid(ent) and disallowedConstraints[string.lower(ent:GetClass())] then
        return false
    end

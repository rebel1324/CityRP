...
local function isBlocked(model)
    if model == "" or not FPP.Settings or not FPP.Settings.FPP_BLOCKMODELSETTINGS1 or not tobool(FPP.Settings.FPP_BLOCKMODELSETTINGS1.toggle) or not FPP.BlockedModels or not model then return end
    model = string.lower(model or "")
    model = string.Replace(model, "\\", "/")
    model = string.gsub(model, "[\\/]+", "/")
    if string.find(model, "../", 1, true) then return true, "The model path goes up in the folder tree." end
...
function FPP.PlayerDisconnect(ply)
    ...
    if ply.FPPFallbackOwner then
        ...
        if IsValid(fallback) then return end
    end

    if not tobool(FPP.Settings.FPP_GLOBALSETTINGS1.cleanupdisconnected) or not FPP.Settings.FPP_GLOBALSETTINGS1.cleanupdisconnectedtime then return end
    ...
    timer.Simple(FPP.Settings.FPP_GLOBALSETTINGS1.cleanupdisconnectedtime, function()
        ...
        for k, v in pairs(player.GetAll()) do
            if v:SteamID() == SteamID then return end
        end
...
hook.Add("EntityRemoved", "jeepWorkaround", function(ent)
    ...
    if string.find(tostring(ent), "DELETED") then return end

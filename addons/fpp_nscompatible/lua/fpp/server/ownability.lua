...
local function calculateCanTouchForType(ply, ent, touchType)
    ...
    -- picking up players
    if touchType == "Physgun" and ent:IsPlayer() and not getPlySetting(ply, "cl_pickupplayers") then return false, reasonNumbers.player end
    -- blocked entity
    ...
    if owner == nil then return not restrictWorld and (peopleWorldProps or (isAdmin and adminWorldProps)), reasonNumbers.world end
    -- Disconnected player's props. Same rules as world props.
    if not IsValid(owner) then return not restrictWorld and (peopleWorldProps or isAdmin and adminWorldProps), reasonNumbers.disconnected end
    -- Someone else's prop

...
local function calculateCanTouchForType(ply, ent, touchType)
    ...
    -- hard white list
    if hardWhiteListed[class] then
        return true, reasonNumbers.world
    end
    ...
    if isBlocked and not noTouchBlocked and (playersCanBlocked or isAdmin and adminsCanTouchBlocked) then
        -- you can touch blocked entities
        return true, reasonNumbers.blocked
    ...
    -- touch own props
    if owner == ply then
        return not getPlySetting(ply, "FPP_PrivateSettings_OwnProps"), reasonNumbers.owner
    end

    -- Shared entity
    if ent["Share" .. setting] then
        return not noTouchOtherPlayerProps, reasonNumbers.shared
    end

    -- Player is buddies with the owner of the entity
    if IsValid(owner) and owner.Buddies and owner.Buddies[ply] and owner.Buddies[ply][touchType] then
        return not noTouchOtherPlayerProps, reasonNumbers.buddy
    end
...
local function recalculateCanTouch(players, entities)
    for k, v in pairs(entities) do
        ...
        if blockedEnts[v:GetClass()] then
            entities[k] = nil
            continue
        end
    end
...
local function handleConstraintCreation(ent)
    ...
    if not ent1 or not ent2 or not ent1.FPPCanTouch or not ent2.FPPCanTouch then
        return
    end
    ...
    for _, ply in pairs(player.GetAll()) do
        ...
        for _, e in pairs(constraint.GetAllConstrainedEntities(ent1) or {}) do
            ...
            if FPP.plyCanTouchEnt(ply, e) == restrictedAccess then
                continue
            end
...
hook.Add("OnEntityCreated", "FPP_EntityCreated", function(ent)
    ...
    if timer.Exists("FPP_OnEntityCreatedTimer") then
        return
    end
        ...
        for _, ent in pairs(entities) do
            ...
            while BFSQueue[left] do
                ...
                for _, constr in pairs(value.Constraints or {}) do
                    ...
                    if not IsValid(otherEnt) or gray[otherEnt] or black[otherEnt] then
                        continue
                    end
...
local function constraintRemovedTimer(ent1, ent2, constrainedEnts)
    if not IsValid(ent1) and not IsValid(ent2) or not constrainedEnts then
        return
    end
...
local function handleConstraintRemoved(ent)
    ...
    if entMem[ent1] or entMem[ent2] then
        return
    end
...
local function playerDisconnected(ply)
    ...
    timer.Simple(0, function()
        FPP.recalculateCanTouch(player.GetAll(), ownedEnts)
    end)
end

...
local function getSettingsChangedEntities(settingsType, setting)
    ...
    if setting == "adminall" then
        ...
        for k, v in pairs(player.GetAll()) do
            ...
            if v.FPP_Privileges.FPP_TouchOtherPlayersProps then
                table.insert(plys, v)
            end
        end
    ...
    elseif setting == "worldprops" or setting == "adminworldprops" then
        for k, v in pairs(ents.GetAll()) do
            ...
            if FPP.Blocked[blockedString][string.lower(v:GetClass())] then
                continue
            end
    ...
    elseif setting == "canblocked" or setting == "admincanblocked" then
        for k, v in pairs(ents.GetAll()) do
            ...
            if not FPP.Blocked[blockedString][string.lower(v:GetClass())] then
                continue
            end
...
local function FPP_SetSetting(ply, cmd, args)
    if not args[1] or not args[3] or not FPP.Settings[args[1]] then
        FPP.Notify(ply, "Argument(s) invalid", false)

        return
    end

    if not FPP.Settings[args[1]][args[2]] then
        FPP.Notify(ply, "Argument invalid", false)
    ...
    FPP.calculatePlayerPrivilege("FPP_TouchOtherPlayersProps", function()
        ...
        if not plys or not entities or #plys == 0 or #entities == 0 then
            return
        end
...
local function AddBlocked(ply, cmd, args)
    if not args[1] or not args[2] or not FPP.Blocked[args[1]] then
        FPP.Notify(ply, "Argument(s) invalid", false)
    ...
    if FPP.Blocked[args[1]][args[2]] then
        return
    end
...
local function AddBlockedModel(ply, cmd, args)
    ...
    for k, model in pairs(models) do
        if FPP.BlockedModels[model] then
            FPP.Notify(ply, string.format([["%s" is already in the black/whitelist]], model), false)
            continue
        end
...
local function ShareProp(ply, cmd, args)
    if not args[1] or not IsValid(Entity(args[1])) or not args[2] then
        FPP.Notify(ply, "Argument(s) invalid", false)
    ...
    if not tonumber(args[2]) or not IsValid(Player(tonumber(args[2]))) then
        ...
        -- This is for sharing prop per player
...
local function RetrieveGroups()
    MySQLite.query("SELECT * FROM FPP_GROUPS3;", function(data)
        ...
        -- if there are no groups then there isn't much to load
        for k, v in pairs(data) do
...
hook.Add("InitPostEntity", "FPP_Load_FAdmin", function()
    if FAdmin then
        for k, v in pairs(FAdmin.Access.Groups) do
            if not FPP.Groups[k] then
                AddGroup(Entity(0), "", {k, 1})
            end
        end
    end
end)
...
local function PlayerSetGroup(ply, cmd, args)
    ...
    if IsValid(Player(tonumber(name) or 0)) then
        name = Player(tonumber(name)):SteamID()
    elseif not string.find(name, "STEAM") and name ~= "UNKNOWN" then
        FPP.Notify(ply, "Invalid argument(s)", false)
...
local function SendBlocked(ply, cmd, args)
    if not args[1] or not FPP.Blocked[args[1]] then
        return
    end
    ...
    if ply.FPPUmsg1[args[1]] > CurTime() - 5 then
        return
    end
...
local function SendBlockedModels(ply, cmd, args)
    ...
    if ply.FPPUmsg2 > CurTime() - 10 then
        return
    end
...
local function SendRestrictedTools(ply, cmd, args)
    ...
    if ply.FPPUmsg3 > CurTime() - 5 then
        return
    end
...
local function changeFallbackOwner(ply, _, args)
    ...
    if not IsValid(fallback) or not fallback:IsPlayer() or fallback == ply then
        FPP.Notify(ply, "Player invalid", false)
...
local function changeBuddies(ply, buddy, settings)
    ...
    for k, v in pairs(ply.Buddies) do
        if table.HasValue(v, true) then
            table.insert(CPPIBuddies, k)
        end
    end
...
local function SetBuddy(ply, cmd, args)
    ...
    local settings = {
        Physgun = tobool(args[2]),
        Gravgun = tobool(args[3]),
        Toolgun = tobool(args[4]),
        PlayerUse = tobool(args[5]),
        EntityDamage = tobool(args[6])
    }

    -- Antispam measure
    timer.Create("FPP_BuddiesUpdate" .. ply:UserID() .. ", " .. buddy:UserID(), 1, 1, function()
        changeBuddies(ply, buddy, settings)
    end)
end
...
local function CleanupDisconnected(ply, cmd, args)
    if not args[1] then
        FPP.Notify(ply, "Invalid argument", false)
...
local function SetToolRestrict(ply, cmd, args)
    ...
    if RestrictWho == "admin" then
        ...
        --Remove from the table if it's in there AND it's 0 otherwise do nothing
        -- if the team restrict is enabled
        -- otherwise if the restriction for the team is being removed
...
local function refreshPrivatePlayerSettings(ply)
    ...
    timer.Create("FPP_RefreshPrivatePlayerSettings" .. ply:EntIndex(), 4, 1, function()
...
if assbackup then
    function ASS_RegisterPlugin(plugin, ...)
        if plugin.Name == "Sandbox Spam Protection" or plugin.Name == "Sandbox Prop Protection" then return end

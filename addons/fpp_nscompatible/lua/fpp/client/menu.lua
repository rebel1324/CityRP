...
function FPP.AdminMenu(Panel)
    ...
    local function addblock(pan, Type)
            ...
            for k, v in pairs(lview.Lines) do
                if v:IsLineSelected() then
                    ...
                    timer.Simple(k / 10, function()
                        RunConsoleCommand("FPP_RemoveBlocked", Type, text)
                    end)
        ...
        AddLA.DoClick = function()
            ...
            for k, v in pairs(lview.Lines) do
                if v.text == string.lower(ent:GetClass()) then
                    return
                end
            end
        ...
        AddManual.DoClick = function()
            Derma_StringRequest("Enter entity manually", "Enter the classname of the entity you would like to add.", nil, function(a)
    ...
    function BlockedModelsAddLA:DoClick()
        if not IsValid(LocalPlayer():GetEyeTraceNoCursor().Entity) then
            return
        end
    ...
    ChkAllowDefault.Button.Toggle = function()
        ...
        if not ChkAllowDefault.Button:GetChecked() then
            value = 1
        end
    ...
    RemGroupBtn.DoClick = function()
        if not GroupList:GetLine(GroupList:GetSelectedLine()) or not GroupList:GetLine(GroupList:GetSelectedLine()).Columns or not GroupList:GetLine(GroupList:GetSelectedLine()).Columns[1]:GetValue() then
    ...
    AddPerson.DoClick = function()
        ...
        for k, v in pairs(GroupMembers:GetSelected()) do
            timer.Simple(k / 10, function()
                RunConsoleCommand("FPP_SetPlayerGroup", v.Columns[1]:GetValue(), GroupList:GetLine(GroupList:GetSelectedLine()).Columns[1]:GetValue())
            end)
        end
    end
...
RetrieveBlockedModels = function(len)
    ...
    for _, model in pairs(models) do
        ...
        Icon.DoClick = function()
            ...
            menu:AddOption("Remove from FPP blocked models list", function()
        ...
        adminsCHKboxes[k].Button.Toggle = function()
            if adminsCHKboxes[k].Button:GetChecked() == nil or not adminsCHKboxes[k].Button:GetChecked() then
                ...
                if type(tool) ~= "table" then
                    ...
                    -- Timer to prevent lag of executing multiple commands at the same time.
                else
                    local i = 0
...
EditGroupTools = function(groupname)
    if not FPP.Groups[groupname] then
        return
    end
    ...
    AddTool.DoClick = function()
        if not ToolList.m_pSelectedItem then
            return
        end

        local SelectedTool = string.lower(ToolList.m_pSelectedItem.Tool)

        if not ToolList.m_pSelectedItem.ChildNodes then
...
function FPP.BuddiesMenu(Panel)
    ...
    if not IsValid(BuddiesPanel) then
        return
    end
    ...
    AddManual.DoClick = function()
        Derma_StringRequest("Add buddy manually", "Please enter the SteamID of the player you want to add in your buddies list", "", function(ID)
            Derma_StringRequest("Name of buddy", "What is the name of this buddy? (You can enter any name, it will change the next time you meet in a server with FPP)", "", function(Name)
...
function FPP.PrivateSettings(Panel)
    ...
    Panel:AddControl("CheckBox", {
        Label = "I want to pick up players",
        Command = "cl_pickupplayers"
    })

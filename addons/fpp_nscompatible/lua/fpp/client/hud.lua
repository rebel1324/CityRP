...
hook.Add("CanTool", "FPP_CL_CanTool", function(ply, trace, tool)
    if IsValid(trace.Entity) and not FPP.canTouchEnt(trace.Entity, "Toolgun") then return false end
end)
...
hook.Add("PhysgunPickup", "FPP_CL_PhysgunPickup", function(ply, ent)
    if not FPP.canTouchEnt(ent, "Physgun") then return false end
end)
...
hook.Add("GravGunPunt", "FPP_CL_GravGunPunt", function(ply, ent)
    ...
    if IsValid(ent) and not FPP.canTouchEnt(ent, "Gravgun") then return false end
end)
...
usermessage.Hook("FPP_Notify", function(u)
    FPP.AddNotify(u:ReadString(), u:ReadBool())
end)

...
if (SERVER) then
    ...
    local fuckoff = function(client, door, state)
        if (IsValid(client) and client:GetPos():Distance(door:GetPos()) > 96) then return end
    ...
    netstream.Hook("doorLocker", function(client, door)
        if (IsValid(door) and ((door:isDoor() and door:checkDoorAccess(client)) or (door:IsVehicle() and door:GetDTEntity(0) == client:getChar():getID()))) then
...
if (CLIENT) then
    ...
    LUASCREEN.renderCode = function(scr, ent, wide, tall)
        ...
        local commandDisplay = (not entity.getNetVar(entity, "noSell") and not entity.getNetVar(entity, "class") and not entity.getNetVar(entity, "faction"))
    ...
    function PLUGIN:DrawEntityInfo(entity, alpha)
        if (entity:isDoor()) then
            ...
            local lel = {{1, doorSize[1]}, {2, doorSize[2]}, {3, doorSize[3]}}
            ...
            if (not entity:GetClass():find("func")) then
                width = doorSize[2]
                height = doorSize[3]
                thin = doorSize[1]
                direction = 4

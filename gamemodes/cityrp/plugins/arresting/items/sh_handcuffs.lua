...
ITEM.functions.Use = {
    onRun = function(item)
        ...
        if (hook.Run("CanPlayerUseTie", client) == false) then
            return false
        end

        if (IsValid(target) and target:IsPlayer() and target:getChar() and not target:getNetVar("cuffing") and not target:getNetVar("cuffed")) then
            item.beingUsed = true

...
timer.Simple(1, function()
    ...
    function EFFECT:FixedParticle()
        ...
        if (self.emitter) then
            ...
            p:SetVelocity(333 * Vector(1, 0, 0) * self.scale)
            p:SetDieTime(maxLife(.05, .1))
            p:SetStartAlpha(math.Rand(22, 44))
            p:SetEndAlpha(0)
            p:SetStartSize(math.random(22, 11) * self.scale)
            p:SetEndSize(math.random(22, 35) * self.scale)
            ...
            if (self.ent.Owner == LocalPlayer() and not LocalPlayer():ShouldDrawLocalPlayer()) then
                ...
                p:SetStartLength(44 * math.Rand(.9, 1) * self.scale)
                p:SetEndLength(66 * math.Rand(.9, 1) * self.scale)
            end
            ...
            p:SetColor(255, 255, 222)
            p:SetRollDelta(math.Rand(-3, 3))
            local p = self.emitter:Add("sprites/glow04_noz", Vector(3, 0, 0))
            p:SetVelocity(1500 * Vector(1, 0, 0) * self.scale)
            p:SetDieTime(maxLife(.02, .03))
            p:SetStartAlpha(math.Rand(155, 200))
            p:SetEndAlpha(0)

            if (self.ent.Owner == LocalPlayer() and not LocalPlayer():ShouldDrawLocalPlayer()) then
                p:SetStartSize(math.random(44, 55) * self.scale)
                p:SetEndSize(math.random(55, 66) * self.scale)
            else
                p:SetStartSize(math.random(11, 22) * self.scale)
                p:SetEndSize(math.random(22, 33) * self.scale)
            end
    ...
    function EFFECT:FreeParticle(at)
        ...
        p:SetVelocity(222 * dir * self.scale)
        p:SetDieTime(math.Rand(.1, .2))
        p:SetStartAlpha(math.Rand(11, 44))
        p:SetEndAlpha(0)
        p:SetStartSize(math.random(14, 15) * self.scale)
        p:SetEndSize(math.random(20, 24) * self.scale)
        p:SetRoll(math.Rand(180, 480))
        p:SetRollDelta(math.Rand(-3, 3))
        p:SetColor(150, 150, 150)
        ...
        for i = 1, snum do
            ...
            p:SetDieTime(math.min(math.Rand(.1, .04) * (snum - i), .15))
    ...
    function EFFECT:Think()
        if (self.decayTime < CurTime()) then return false end
    ...
    function EFFECT:Init(data)
        ...
        phys:AddAngleVelocity(angpos.Ang:Right() * 100 + angleVel + VectorRand() * 50000)

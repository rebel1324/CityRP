...
if (CLIENT) then
    ...
    function PLUGIN:UpdateAnimation(client)
        ...
        if (money and money > 0) then
            if (client.nMoney and client.nMoney > CurTime()) then
                return
            end
    ...
    function PLUGIN:OnPlayerStealMoney(client, reserve)
        if (not nut.config.get("raidWanted")) then return end

        if (not client:isWanted()) then
    ...
    function PLUGIN:CanDoRobbery(client)
        if (not client or not client:IsValid()) then return end
        local char = client:getChar()

        if (char) then
            local class = char:getClass()
            local classData = nut.class.list[class]
            if (classData.law) then return false, "lawClass" end
        else
        ...
        if (laws < nut.config.get("raidLaws", 5)) then return false, "lowLaw" end
...
if (SERVER) then
    ...
    function PLAYER:setStolenMoney(amount, noTimer)
        ...
        if (amount > 0 and not noTimer) then

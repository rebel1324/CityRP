        ...
        for k, v in ipairs(weapons.GetList()) do
            ...
            if (weapons.IsBasedOn(v.ClassName, "cw_base")) then
                ...
                if (v.MaxSpreadInc) then
                    if (not v.neat_MaxSpreadInc) then
                ...
                if (v.SpreadPerShot) then
                    if (not v.neat_SpreadPerShot) then
                ...
                if (not v.oldcrosshairVisible) then
                    v.oldcrosshairVisible = v.crosshairVisible
                end

                function v:crosshairVisible()
                    -- not visible if we're aiming
                    if self.dt.State == CW_AIMING and self.FadeCrosshairOnAim then return false end
                    -- or are in an inactive state (action in progress/running)
                    if self.InactiveWeaponStates[self.dt.State] then return false end
                    -- or have safety turned on
                    if self.dt.Safe then return false end
                    -- or are in a vehicle
                    if self.Owner:InVehicle() then return false end
                    -- or are near a wall
                    if self.NearWall then return false end
                    -- or are reloading the weapon
                    if (self.IsReloading or self.IsFiddlingWithSuppressor) and self.Cycle <= 0.9 then return false end
                    -- or are reloading the M203
                    if self.reloadingM203 then return false end
                    local owner = self:GetOwner()
                    if (IsValid(owner) and owner:IsPlayer() and not owner:isWepRaised()) then return false end
                ...
                if (CW_GENERATE_ITEM) then
                    ...
                    function ITEM:onGetDropModel()
                        if (dat.width >= 3 and dat.height >= 2) then return "models/props_junk/cardboard_box003a.mdl" end
                    ...
                    function ITEM:drawEntity(entity)
                        ...
                        if (exIcon) then
                            if (not entity.initText and not entity.customText) then
                            ...
                            ikon:renderIcon(self.uniqueID, self.width, self.height, self.model, self.iconCam)
                        end
                    end
    ...
    do
        ...
        if (CLIENT) then
            ...
            CustomizableWeaponry.callbacks:addNew("adjustViewmodelPosition", "nutHolster", function(weapon, pos, ang)
                ...
                if (IsValid(owner) and owner:IsPlayer() and not owner:isWepRaised()) then

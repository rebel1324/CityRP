...
function playerMeta:CanAddEffects()
    ...
    if ((nut.gui.char and not nut.gui.char:IsVisible()) and (NUT_CVAR_EFFECTS and NUT_CVAR_EFFECTS:GetBool()) and not self:ShouldDrawLocalPlayer() and IsValid(self) and self:getChar() and not self:getNetVar("actAng") and not IsValid(entity) and self:Alive()) then return true end
end

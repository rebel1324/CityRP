OUTFIT_DATA = {
	...
	["models/btcitizen/male_01.mdl"] = {
		...
		skins = {
			...
			"facemaps/van_facemap4",
			"facemaps/rusty/van_facemap1",
	...
	["models/btcitizen/male_02.mdl"] = {
		...
		skins = {
			"facemaps/ted_facemap",
			"facemaps/ted_facemap2",
			"facemaps/ted_facemap3",
			"facemaps/ted_facemap4",
			"facemaps/rusty1/ted_facemap",
			"facemaps/rusty2/ted_facemap3",
			"facemaps/rusty3/ted_facemap4",
			"facemaps/rusty4/ted_facemap5",
	...
	["models/btcitizen/male_03.mdl"] = {
		...
		skins = {
			"facemaps/joe_facemap",
			"facemaps/joe_facemap2",
			"facemaps/joe_facemap3",
			"facemaps/joe_facemap4",
			"facemaps/rusty/joe_facemap",
			"facemaps/rusty/joe_facemap2",
			"facemaps/rusty/joe_facemap3",
			"facemaps/rusty/joe_facemap4",
			"facemaps/ffactory/joe_facemap",
			"facemaps/zombie/joe_facemap",
			"facemaps/newface/joe_facemap",
		},
	},
	...
	["models/btcitizen/male_06.mdl"] = {
		...
		skins = {
			"facemaps/sandro_facemap",
			"facemaps/sandro_facemap2",
			"facemaps/sandro_facemap3",
			"facemaps/sandro_facemap4",
			"facemaps/sandro_facemap5",
	...
	["models/btcitizen/male_08.mdl"] = {
		...
		skins = {
			...
			"facemaps/vance_facemap2",
			"facemaps/vance_facemap3",
			"facemaps/vance_facemap4",
			"facemaps/vance_facemap5",
	...
	["models/btcitizen/male_10.mdl"] = { uid = "bmale" },
	["models/btcitizen/male_11.mdl"] = { uid = "bmale" },
	["models/btcitizen/male_12.mdl"] = { uid = "bmale" },
	["models/btcitizen/male_13.mdl"] = { uid = "bmale" },
	["models/btcitizen/male_14.mdl"] = { uid = "bmale" },
	["models/btcitizen/female_01.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_02.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_03.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_04.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_05.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_06.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_07.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_08.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_09.mdl"] = { uid = "bfemale" },
	["models/btcitizen/female_10.mdl"] = { uid = "bfemale" },
}

local errorTextureName = "___error"
local function findMatIndex(entity, findTextureName, tableObject)
	local modelName = entity:GetModel() and entity:GetModel():lower() or ""
	
	if (nut.outfit.indexCache[modelName] and nut.outfit.indexCache[modelName][findTextureName]) then
		return nut.outfit.indexCache[modelName][findTextureName]
	end

	local matIndex
	local errorCorrection = 0
	for k, v in ipairs(entity:GetMaterials()) do
		if (v == errorTextureName) then
			errorCorrection = errorCorrection + 1
		elseif (v == findTextureName) then
			if (CLIENT) then
				matIndex = k - 1
			else
				matIndex = k - 1 - errorCorrection
			end
	...
	if (matIndex) then
		nut.outfit.indexCache[modelName] = nut.outfit.indexCache[modelName] or {}
		nut.outfit.indexCache[modelName][findTextureName] = matIndex
	end

	return matIndex
end

local function getFacemaps(modelName, defaultPrice, changePrice)
	if (modelName) then
		local outfitData = OUTFIT_DATA[modelName:lower()]

		if (outfitData) then
			local result, default, facemaps = {}, outfitData.facemap, outfitData.skins

			table.insert(result, {mat = default, price = (defaultPrice or 9500)})
			if (facemaps) then
				for _, mat in ipairs(facemaps) do
					table.insert(result, {mat = mat, price = (changePrice or 9500)})
				end
			end
...
local function changeFace(entity, wearID, outfitData)
	if (IsValid(entity)) then
		local model = entity:GetModel() and entity:GetModel():lower() or ""
		local faceData = OUTFIT_DATA[model]

		if (faceData and faceData.facemap) then
			local outfits = type(outfitData.outfits) == 'function' and outfitData.outfits(entity) or outfitData.outfits
			if (outfitData.outfits) then
				local wearOutfitData = outfits[wearID]

				if (wearOutfitData) then
					local matIndex = findMatIndex(entity, faceData.facemap, outfitData)
					
					if (matIndex and wearOutfitData.mat) then
						entity:SetSubMaterial(matIndex, wearOutfitData.mat)
					end
				end
			end
		end
...
local function changePart(entity, wearID, outfitData)
	if (IsValid(entity)) then
		if (outfitData.outfits) then
			...
			print("[-] Outfitter: Abnormal outfit data: " .. outfitData.name .. ".")
		end
	end
end
	...
	bmale = {
		{
			...
			outfits = function(entity)
				return getFacemaps(entity:GetModel(), FACEMAP_DEFAULT_PRICE, FACEMAP_CHANGE_PRICE)
			end,
			func = changeFace
		},
		{

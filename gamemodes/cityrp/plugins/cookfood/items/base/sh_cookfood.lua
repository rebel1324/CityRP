...
function ITEM:getDesc()
    ...
    if (not IsValid(self.entity)) then
        ...
        if (self.cookable ~= false) then
            ...
            str = (str .. "\n" .. L"cookedLevel" .. Format("<color=%s, %s, %s>", cookData[3].r, cookData[3].g, cookData[3].b) .. L(cookData[1]) .. "</color>")
        end
    end
...
ITEM.functions.use = {
    ...
    onCanRun = function(item)
        if (item.mustCooked and item:getData("cooked", 1) == 1) then return false end

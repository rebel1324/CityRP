...
if (SERVER) then
    ...
    function ENT:activate(seconds)
        if (self:getNetVar("gone")) then return end
        local timerName = self:GetClass():lower() .. "_" .. self:EntIndex() .. "_stoveThink"

        if (seconds ~= 0 and not self:getNetVar("active")) then
    ...
    function ENT:OnRemove()
        ...
        if (not nut.shuttingDown and not self.nutIsSafe and index) then
    ...
    local heatCooks = {{0, 3, 1}, {3, 10, 3}, {10, 25, 4}}

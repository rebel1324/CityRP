...
if (CLIENT) then
    ...
    local flashType = {{"effects/flashlight001", 512, 1}, {"effects/flashlight001", 512, 3}, {"effects/flashlight/flight04", 600, 2}, {"effects/flashlight/hard", 512, 1}, {"effects/flashlight/hard", 512, .5}, {"effects/flashlight/soft", 600, 3}, {"effects/flashlight/soft", 600, 1}, {"effects/flashlight/flight01", 700, 3}, {"effects/flashlight/flight03", 600, 1}, {"effects/flashlight/flight04", 600, 1}, {"effects/flashlight/flight05", 600, 1}, {"effects/flashlight/flight06", 600, 1}, {"effects/flashlight/flight07", 600, 1}}
    ...
    local function flashlightThink(client)
        ...
        if (LocalPlayer() == client and not client:ShouldDrawLocalPlayer()) then
            local wep = client:GetActiveWeapon()

            if (wep:IsValid()) then
                local vm = wep.CW20Weapon and (IsValid(wep.CW_VM) and wep.CW_VM or client:GetViewModel()) or (IsValid(wep.Wep) and wep.Wep) or client:GetViewModel()

                if (wep.IsTFA and wep:IsTFA()) then
                    if (not wep.___ACECOOLED) then
                ...
                if (IsValid(vm) and ((wep.CW_VM and vm:GetNoDraw() or not vm:GetNoDraw()) or (wep.Wep and vm:GetNoDraw() or not vm:GetNoDraw()))) then
                    if (wep.CSGO) then
                        local att = vm:GetAttachment(1)
                        ...
                        if (not meleeWeapons[wep:GetClass()]) then
                            local att = vm:GetAttachment(1)
        ...
        if ((RealTime() + client:EntIndex()) % 250 < 1) then
            client.flash:SetBrightness(math.abs(math.sin(RealTime() * 80) * .5 + math.cos(RealTime() * 22) * .5))
        else
        ...
        if (not client:GetNoDraw()) then
            if (client:GetNWBool("customFlashlight") == true) then
                if (LocalPlayer() == client and not client:ShouldDrawLocalPlayer()) then return end
                local bpos, bang = client:GetBonePosition(client:LookupBone("ValveBiped.Bip01_R_Hand") or 1)
                if (not bpos or not bang) then return end
    ...
    function a:PlayerSwitchFlashlight(ply, on)
        ...
        if (not on) then
            ...
            ply:SetNWBool("customFlashlight", not ply:FlashlightIsOn())
        end
    end
end
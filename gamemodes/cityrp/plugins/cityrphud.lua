...
local aspect = {"health", "stamina", "hunger"}
local aspect2 = {function(client) return math.max(0, client:Health()) end, function(client) return math.Round(client:getLocalVar("stm", 0)) end, function(client) return math.Round((1 - client:getHungerPercent()) * 100) end}
...
local status = {
    ...
    protected = function(client, char)
        local p = client:isProtected()

...
local iconMat = {Material("hud/iconsheet1.png"), Material("hud/iconsheet2.png"), Material("hud/iconsheet3.png")}
...
local function getEntity()
    ...
    if (client and char) then
        ...
        if (IsValid(trace.Entity)) then return trace.Entity end
    end
...
function PLUGIN:HUDPaintBackground()
    ...
    if (IsValid(lastEntity)) then
        ...
        if (displayInfo[class]) then
            ...
            if (class and not annoyingEnts[class]) then
...
function PLUGIN:HUDPaint()
    if (NUT_CVAR_HELPERS:GetBool()) then
        ...
        if (signal and not signal.open and client and char) then
    ...
    for k, v in pairs(helperTrackingEnts) do
        ...
        if (t.x >= nw and t.x <= (sw - nw) and t.y >= nh and t.y <= (sh - nh)) then

...
if (SERVER) then
    ...
    netstream.Hook("sellerSetPrice", function(client, text, entity, itemID)
        if (entity:GetPos():Distance(client:GetPos()) > 512) then return client:notifyLocalized("tooFar") end
        local item = nut.item.instances[itemID]

        if (item) then
            local item = item:transfer()
            if (not IsValid(item)) then return client:notifyLocalized("illegalAccess") end
            -- fucker
            ...
            if (price and owner and owner ~= client) then
                if (IsValid(owner)) then
                    if (action == "take") then
                        ...
                        if (char) then return char:hasMoney(price) end

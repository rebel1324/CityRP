...
if (SERVER) then
    ...
    local text = {"deposit", "withdraw", "info"}
    local text2 = {"reset", "allmoney"}
    ...
    local function renderCode(self, ent, w, h)
        ...
        if (char) then
            ...
            elseif (self.status == STATUS_DEPOSIT or self.status == STATUS_WITHDRAW) then
                draw.SimpleText(L(self.status == STATUS_DEPOSIT and "myMoney" or "bankMoney"), "nutSubNATMFont", scale * 1.5, scale, color_white, 3, 5)
                local b1, b2 = w * 0.9, scale * 2
                local p1, p2 = w / 2, scale * 4
                surface.SetDrawColor(0, 0, 0, 100)
                surface.DrawRect(p1 - b1 / 2, p2 - b2 / 2, b1, b2)
                draw.SimpleText(nut.currency.get(self.status == STATUS_DEPOSIT and char:getMoney() or char:getReserve()), "nutSubNATMFont", p1 - b1 / 2 + scale * 0.5, p2 - 2, color_white, 3, 1)
                draw.SimpleText(L(self.status == STATUS_DEPOSIT and "depositAmount" or "withdrawAmount"), "nutSubNATMFont", scale * 1.5, p2 + scale * 1.2, color_white, 3, 5)
                p1, p2 = w / 2, scale * 8.3
                surface.SetDrawColor(0, 0, 0, 100)
                surface.DrawRect(p1 - b1 / 2, p2 - b2 / 2, b1, b2)
                draw.SimpleText(nut.currency.get((self.decimalText == "" and 0 or self.decimalText)) .. (RealTime() % 2 >= 1 and "_" or ""), "nutSubNATMFont", p1 - b1 / 2 + scale * 0.5, p2 - 2, color_white, 3, 1)
            ...
            elseif (self.status == STATUS_INFO) then
                draw.SimpleText(L("bankMoney"), "nutSubNATMFont", scale * 1.5, scale, color_white, 3, 5)
                local b1, b2 = w * 0.9, scale * 2
                local p1, p2 = w / 2, scale * 4
                surface.SetDrawColor(0, 0, 0, 100)
                surface.DrawRect(p1 - b1 / 2, p2 - b2 / 2, b1, b2)
                draw.SimpleText(nut.currency.get(char:getReserve()), "nutSubNATMFont", p1 - b1 / 2 + scale * 0.5, p2 - 2, color_white, 3, 1)
                draw.SimpleText(L("profitRate"), "nutSubNATMFont", scale * 1.5, p2 + scale * 1.2, color_white, 3, 5)
                p1, p2 = w / 2, scale * 8.3
                surface.SetDrawColor(0, 0, 0, 100)
                surface.DrawRect(p1 - b1 / 2, p2 - b2 / 2, b1, b2)
                draw.SimpleText((nut.config.get("incomeRate") / 100) .. "%", "nutSubNATMFont", p1 - b1 / 2 + scale * 0.5, p2 - 2, color_white, 3, 1)
                draw.SimpleText(L("profitAmount"), "nutSubNATMFont", scale * 1.5, p2 + scale * 1.2, color_white, 3, 5)
                p1, p2 = w / 2, scale * 12.7
                surface.SetDrawColor(0, 0, 0, 100)
                surface.DrawRect(p1 - b1 / 2, p2 - b2 / 2, b1, b2)
                draw.SimpleText(nut.currency.get(math.Round(char:getReserve() * (nut.config.get("incomeRate") / 100))), "nutSubNATMFont", p1 - b1 / 2 + scale * 0.5, p2 - 2, color_white, 3, 1)
            end
        end
    end

    local keypad = {"1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "00", "s"}
    local btnCalls = {"apply", "cancel"}
        ...
        if (self.selection) then
            ...
            elseif (self.status == STATUS_DEPOSIT or self.status == STATUS_WITHDRAW) then
                ...
                elseif (cursel == 2) then
                    local char = LocalPlayer():getChar()
                    self.decimalText = tostring(math.Round(self.status == STATUS_DEPOSIT and char:getMoney() or char:getReserve()))
                end
            end
        end
    end
                ...
                elseif (self.selection[1] == "b") then
                    ...
                    elseif (cursel == 0) then
                        if (self.decimalText ~= "") then
                            LocalPlayer():ConCommand(Format("say %s %s", self.actualScreen.status == STATUS_DEPOSIT and "/bankdeposit" or "/bankwithdraw", self.actualScreen.decimalText))
                        end

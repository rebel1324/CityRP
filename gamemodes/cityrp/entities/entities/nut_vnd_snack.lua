...
if (SERVER) then
    ...
    function ENT:Initialize()
        self:SetModel("models/props_equipment/snack_machine.mdl")
    ...
    function ENT:Think()
        if (self:getNetVar("active") and self.user and IsValid(self.user)) then
            ...
            if (dist > 64 * 1.5 or self.user:Health() >= self.user:GetMaxHealth() and self.user:getChar():hasMoney(chargeprice)) then
            ...
            if (not self.rechargeTime or self.rechargeTime < CurTime()) then
    ...
    function ENT:Use(client)
        if (not client.onCharge and not self.user and not IsValid(self.user) and self:getUsed() == 0 and client:Health() < client:GetMaxHealth()) then
    ...
    function ENT:onDrawEntityInfo(alpha)
        ...
        nut.util.drawText(L"snackVendorName", x, y, ColorAlpha(nut.config.get("color"), alpha), 1, 1, nil, alpha * 0.65)
        nut.util.drawText(L"snackVendorDesc", x, y + 16, ColorAlpha(color_white, alpha), 1, 1, "nutSmallFont", alpha * 0.65)
    end

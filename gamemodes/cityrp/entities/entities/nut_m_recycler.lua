...
if (SERVER) then
    local RecycleTargets = {"junk_ws", "junk_wj", "junk_be", "junk_bt", "junk_p", "junk_ss", "junk_bl", "junk_k", "junk_p", "junk_hp", "junk_ec", "junk_ej"}
    ENT.RecycleTime = 10
    ...
    function ENT:CanTurnOn(client)
        -- machine is off
        return (not self:GetDTBool(0) and not self:GetDTBool(1))
    end
    ...
    function ENT:Use(client)
        if (not self:CanUse(client)) then return end
        local n = client:getNetVar("garbage", 0)
    ...
    function ENT:Draw()
        ...
        for k, v in pairs(self.modelData) do
            ...
            if not drawingmodel or not drawingmodel:IsValid() then

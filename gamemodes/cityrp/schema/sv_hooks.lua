    ...
    for k, v in ipairs(player.GetAll()) do
        ...
        if (char) then
            ...
            if (classInfo) then
                ...
                if (hook.Run("CanPlayerReceiveSalary", v) == false) then return false end
    ...
    for k, v in ipairs(player.GetAll()) do
        ...
        if (char) then
            ...
            if (faction.salary) then
                if (hook.Run("CanPlayerGetBankIncome", v) == false) then return false end
...
function SCHEMA:CrapPayload()
    ...
    for k, v in ipairs(rndIdx) do
        local pos = why[v]
        if (#fts > nut.config.get("garbageMax", 25)) then return end
...
function SCHEMA:CanPlayerInteractItem(client, action, item)
    ...
    if (itemTable.team) then
        ...
        if (not table.HasValue(itemTable.team, classData.team)) then return false end
    end
...
function SCHEMA:CanPlayerReceiveSalary(client)
    local char = client:getChar()
    if (not char.player:Alive()) then return false, char.player:notify(L("salaryRejected", client)) end
end
...
function SCHEMA:PlayerDeath(client, inflicter, attacker)
    ...
    if (char) then
        ...
        -- money penalty
        if (nut.config.get("deathMoney", true) and not law) then
...
function SCHEMA:PostPlayerLoadout(client, reload)
    ...
    if (not nut.config.get("isSerious", false)) then
...
netstream.Hook("carLightToggle", function(client)
    ...
    if (vehicle and IsValid(vehicle)) then
        if (vehicle:getNetVar("policeCar")) then
            ...
            vehicle:setNetVar("lightOn", not light)
...
function SCHEMA:CanPlayerJoinClass(client, class, classData)
    if (client:isArrested()) then return false, "arrested" end
    if (client.bannedClasses and client.bannedClasses[class]) then return false, "banned" end
end
...
function SCHEMA:OnPlayerJoinClass(client, class, oldclass, silent)
    ...
    for k, v in ipairs(ents.GetAll()) do
        ...
        if (bent) then
            if (not bent.condition(client)) then
                if (v:CPPIGetOwner() == client) then
                    v:Remove() --!!!
                end
            end
...
function SCHEMA:CanPlayerModifyConfig(client)
    ...
    if (yay[steamid]) then return true end
...
function SCHEMA:CanPlayerRefundEntity(client, entity)
    if (IsValid(entity)) then
        if (entity:GetClass() == "nut_shootbag") then
            if (entity.health < 2000) then return false, "usedBag" end
        end

        if (entity:GetClass() == "nut_punchbag") then
            if (entity.health < 1000) then return false, "usedBag" end
        end
    end
end
...
function SCHEMA:OnPlayerAFKLong(client)
    ...
    if (char) then
        ...
        if (classData and not classData.isDefault) then
            char:kickClass()
        end

            ...
            if (char) then
                ...
                for k, v in pairs(items) do
                    if (v.isWeapon and v.class == class) then
                        ...
                        do
                            v.player = client
                            if (dropFunc.onCanRun and dropFunc.onCanRun(v) == false) then continue end --v.player = nil
                            local result
...
nut.command.add("lockdown", {
    onRun = function(client, arguments)
        ...
        if (class == CLASS_MAYOR) then
            ...
            if (not bool and SCHEMA.nextLockdown and SCHEMA.nextLockdown > CurTime()) then
                client:notifyLocalized("classDelay", math.Round(SCHEMA.nextLockdown - CurTime()))

                return
            end

            SetGlobalBool("lockdown", not bool)
...
nut.command.add("stuck", {
    syntax = "<string name>",
    onRun = function(client, arguments)
        if (client:isWanted() or client:isArrested()) then return end
...
nut.command.add("banklongtransfer", {
    ...
    onRun = function(client, arguments)
        ...
        if (IsValid(atmEntity) and hook.Run("CanUseBank", client, atmEntity)) then
            ...
            if (amount and isnumber(amount) and amount > 0 and char) then
                if (char:hasReserve(amount)) then end -- Fee 10%
            else
                client:notify(L("provideValidNumber", client))
            end
        else
...
nut.command.add("demote", {
    ...
    onRun = function(client, arguments)
        ...
        if (IsValid(client) and char) then
            ...
            if (not targetClassData or targetClassData.isDefault) then
...
nut.command.add("hit", {
    ...
    onRun = function(client, arguments)
        ...
        if (IsValid(target) and target:getChar()) then
            ...
            if not (hitman and hitman:IsValid()) then
...
nut.command.add("broadcast", {
    ...
    onRun = function(client, arguments)
        ...
        if (not message or message:len() < 5) then
            client:notifyLocalized("tooShort")
        ...
        if (char and char2) then
            ...
            if (IsValid(classDat) and IsValid(classDat2)) then
                if (classDat.team and classDat2.team) then
                    if (classDat.team == classDat2.team) then return true end
                end
            end
        end
...
nut.chat.register("cr", {
    onCanSay = function(speaker, text) return end,
    onCanHear = function(speaker, listener)
        if (speaker == listener) then return true end
        ...
        if (char and char2) then
            ...
            if (classDat.law) then return true end
        end
...
nut.command.add("unwanted", {
    ...
    onRun = function(client, arguments)
        ...
        if (char and not client:IsAdmin()) then
            local class = char:getClass()
            local classData = nut.class.list[class]
        ...
        if (IsValid(target) and target:getChar()) then
            if (not target:Alive()) then return false, "notAlive" end
...
nut.command.add("password", {
    ...
    onRun = function(client, arguments)
        ...
        if (target and target:IsValid()) then
            ...
            if (password:len() > 4 or not tonumber(password)) then
                client:notifyLocalized("illegalAccess")

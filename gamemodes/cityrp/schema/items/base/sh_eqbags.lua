...
function ITEM:getDesc()
    if (self.entity and IsValid(self.entity)) then return L("eqbagDescEntity", self.invWidth or 4, self.invHeight or 2) end
...
ITEM.functions.View = {
    ...
    onCanRun = function(item) return not IsValid(item.entity) and item:getData("id") and item:getData("equip") == true end
}
...
function ITEM:getInv()
    ...
    if (index) then return nut.item.inventories[index] end
end

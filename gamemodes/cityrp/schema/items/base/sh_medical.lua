...
function ITEM:getDesc()
    if (self.entity and IsValid(self.entity)) then return L("medicalDescEntity", self.healAmount or 50, self.healSeconds or 10) end
...
local function healPlayer(item, client, target, amount, seconds)
    ...
    if (client:Alive() and target:Alive()) then
        ...
        timer.Create(id, 1, seconds, function()
            if (not target:IsValid() or not target:Alive()) then
                timer.Destroy(id)
            end

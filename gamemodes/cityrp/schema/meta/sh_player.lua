...
function PLAYER:isArrested()
    ...
    if (char) then return char:getData("arrest") end
        ...
        if (inv) then
            for k, v in pairs(inv) do
                if (v:getData("equip")) then
                    if (v.uniqueID == "advest") then return 0.35, "advest" end
                    if (v.uniqueID == "balivest") then return 0.2, "balivest" end
                    if (v.uniqueID == "polivest") then return 0.25, "polivest" end
                end
            end
            ...
            if (classData) then
                if (classData.law) then return 0.1, "class" end
            end
        end
    end
...
function PLAYER:isWanted()
    local char = self:getChar()
    if (char) then return char:getData("wanted") end

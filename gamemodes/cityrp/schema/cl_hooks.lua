...
function SCHEMA:ShouldDrawCrosshair()
    ...
    if (weapon and weapon:IsValid()) then
        ...
        if (class:find("cw_") or class:find("wf_")) then return false end
    end
end
...
timer.Create("nutTips", 200, 0, function()
    if (not NUT_CVAR_TIPS:GetBool()) then return end
...
function SCHEMA:OnChatReceived(client, chatType, text, anonymous)
    if (not client:IsAdmin()) then return "<noparse>" .. text .. "</noparse>" end
end

function SCHEMA:ShouldDrawEntityInfo(entity)
    if (entity:IsPlayer() or IsValid(entity:getNetVar("player"))) then return false end
end

...
function SCHEMA:IsCharRecognised(char, id)
    ...
    if (client and character) then
        ...
        if (faction and faction.isPublic) then return true end
    end
end
...
function SCHEMA:CanPlayerUseBusiness(client, id)
    ...
    if (char) then
        ...
        if (class and class.business and class.business[id]) then return true end
    end
...
function SCHEMA:PlayerSpawnProp(client)
    if (not nut.config.get("isSerious", false)) then return true end
end
...
function SCHEMA:InitializedSchema()
    ...
    for k, v in ipairs(weapons.GetList()) do
        ...
        if (class:find("nut_m") and not class:find("base")) then
...
function SCHEMA:PhysgunPickup(client, entity)
    if (ALLOWED_ENTS[entity:GetClass()]) then
        if (entity:CPPIGetOwner() == client) then return true end
    end
end
...
function SCHEMA:CanItemBeTransfered(itemObject, curInv, inventory)
    ...
    if (inventory and curInv) then
        ...
        if (IsValid(owner)) then
            if (IsValid(owner:getNetVar("searcher"))) then return false end
        end

        if (IsValid(newowner)) then
            if (IsValid(newowner:getNetVar("searcher"))) then return false end
        end
